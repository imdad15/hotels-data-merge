...
async function fetchAndCacheHotels() {
  try {
    const adapters = [
      {
        supplier: "acme",
        adapter: new AcmeAdapter(SUPPLIERS.acme),
        enabled: SUPPLIERS.acme.enabled,
      },
      {
        supplier: "patagonia",
        adapter: new PatagoniaAdapter(SUPPLIERS.patagonia),
        enabled: SUPPLIERS.patagonia.enabled,
      },
      {
        supplier: "paperflies",
        adapter: new PaperfliesAdapter(SUPPLIERS.paperflies),
        enabled: SUPPLIERS.paperflies.enabled,
      },
    ].filter((a) => a.enabled);
...
// Run only outside tests
if (process.env.NODE_ENV !== "test") {
  cron.schedule("*/10 * * * *", fetchAndCacheHotels);
}

export { fetchAndCacheHotels };
