...
export function mergeHotels(
  allHotels: { supplier: string; hotels: Hotel[] }[],
): Hotel[] {
  ...
  flatHotels.forEach(({ hotel, supplier }) => {
    ...
    if (!existingEntry) {
      ...
      const mergedHotel = mergeTwoHotels(
        existingEntry.hotel,
        hotel,
        existingEntry.supplier,
        supplier,
      );
...
export function applyRules(hotel: Hotel): Hotel {
  ...
  copy.amenities.general = filterAmenities(
    copy.amenities?.general || [],
    rulesConfig.amenities.validGeneral,
  );
  copy.amenities.room = filterAmenities(
    copy.amenities?.room || [],
    rulesConfig.amenities.validRoom,
  );
...
function mergeTwoHotels(
  ...
  merged.name = selectBySupplierPriority(
    existing.name,
    incoming.name,
    existingSupplier,
    incomingSupplier,
    rulesConfig.name.preference,
  );
  ...
  merged.amenities.general = Array.from(
    new Set([
      ...(existing.amenities?.general || []),
      ...(incoming.amenities?.general || []),
    ]),
  );
  merged.amenities.room = Array.from(
    new Set([
      ...(existing.amenities?.room || []),
      ...(incoming.amenities?.room || []),
    ]),
  );
  ...
  merged.images.rooms = [
    ...
    ...(incoming.images?.rooms || []),
  ];
  merged.images.site = [
    ...(existing.images?.site || []),
    ...(incoming.images?.site || []),
  ];
  merged.images.amenities = [
    ...(existing.images?.amenities || []),
    ...(incoming.images?.amenities || []),
  ];
...
export function selectBySupplierPriority(
  ...
  if (incomingValue && incomingIndex < existingIndex)
...
export function filterAmenities(
  ...
  const originalByNormalized = Object.fromEntries(
    validList.map((v) => [normalizeAmenity(v), v]),
  );
