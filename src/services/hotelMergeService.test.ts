import { mergeHotels, selectBySupplierPriority, filterAmenities, applyRules } from './hotelMergeService';
...
describe('hotelMergeService', () => {
  ...
  describe('mergeTwoHotels', () => {
    it('should merge amenities from both hotels', () => {
      ...
      expect(result.amenities?.general).toEqual(["business center", "wifi", "gym"]);
  ...
  describe('mergeHotels', () => {
    it('should merge hotels from different suppliers', () => {
      ...
      expect(result[0].amenities?.general).toEqual(['business center', 'wifi', 'gym']);
